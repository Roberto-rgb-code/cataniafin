// landing-page/src/views/EspecialDetailDestacado.jsx
import React, { useState, useEffect } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import axios from 'axios';
import './EspecialDetailDestacado.css';

const EspecialDetailDestacado = () => {
  const { id } = useParams();
  ...
  const apiUrl = import.meta.env.VITE_ESPECIALES2_API_URL;

  useEffect(() => {
    const fetchEspecial = async () => {
      setLoading(true);
      try {
        const res = await axios.get(`${apiUrl}/api/especiales-destacados/${id}`, {
          headers: { Accept: 'application/json' },
        });
        setEspecial(res.data);
      } catch (err) {
        setError('Error al obtener el especial destacado: ' + (err.response?.data?.message || err.message));
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <div className="especial-detalle-container"><p>Cargando...</p></div>;
  if (error) return (
    <div className="especial-detalle-container error">
      <p>{error}</p>
      <button className="btn" onClick={() => navigate('/especiales-destacados')}>Volver al catálogo</button>
    </div>
  );
  if (!especial) return (
    <div className="especial-detalle-container error">
  ...
  return (
    <div className="especial-detalle-container">
      <header className="section-header">
        ...
        <h1 className="section-title">Detalles del Producto Especial Destacado</h1>
      </header>
      ...
      <div className="especial-detail-content">
          ...
          >
            ...
            {isZoomed && (
              <div className="zoom-indicator">
                Zoom: {zoomStep + 1}/{zoomLevels.length} — Click para más
              </div>
            )}
          </div>
        </div>
        ...
        <div className="especial-info">
          ...
          <button className="btn" onClick={() => navigate('/especiales-destacados')}>
            Volver al catálogo
          </button>
        </div>
      </div>
...
export default EspecialDetailDestacado;
